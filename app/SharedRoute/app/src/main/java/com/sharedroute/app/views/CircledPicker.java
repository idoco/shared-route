...
public class CircledPicker extends View {
    ...
    private void drawCenteredText(Canvas canvas) {
        ...
        mPaint.setShadowLayer(7,7,7,Color.BLACK);
        ...
        switch (mPickerMode) {
            ...
            case PERCENT:
               ...
               break;
            case PASSENGERS:
               centerLabel = getPassengersString();
               break;
            case NUMERIC:
    ...
    private String getPassengersString() {
        int passengers = (int) getValue();
        if (passengers == 0) {
            return "How Many Passengers?";
        }else if (passengers == 1) {
            return String.valueOf(passengers) + " Passenger";
        } else {
            return String.valueOf(passengers) + " Passengers";
        }
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if(pickerMode != null) {
            mPickerMode =
                    pickerMode.equalsIgnoreCase("hours") ? PickerMode.HOURS_AND_SECONDS :
                            pickerMode.equalsIgnoreCase("minutes") ? PickerMode.TIME_OF_DAY :
                                    pickerMode.equalsIgnoreCase("numeric") ? PickerMode.NUMERIC :
                                            pickerMode.equalsIgnoreCase("percent") ? PickerMode.PERCENT :
                                                    PickerMode.PASSENGERS;
        } else {
